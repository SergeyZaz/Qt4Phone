...
void CSettingsDlg::readIni()
{
	...
	ui.IDS_UNAMESTR->setText(QString::fromUtf8((const char*)p_endpoint->GetLocalUserName()));
	...
	else
	{
		...
		case 0:
			...
			break;
		case 1:
			ui.IDS_GKHOSTLOCSTR->setChecked(true);
			break;
		case 2:
			ui.IDS_GKIDLOCSTR->setChecked(true);
			break;
		default:
			break;
		}
	...
	{
		tText = QString((const char*)vdevices[i]);
		//tText = QString::fromUtf8((const char*)vdevices[i]);
		ui.IDS_DEVSTR->addItem(tText);
	}
	
	PStringArray fakedevice = PVideoInputDevice::GetDriversDeviceNames("FakeVideo");
	for (i = 0; i < fakedevice.GetSize(); i++)
	{
		tText = QString((const char*)fakedevice[i]);
		//tText = QString::fromUtf8((const char*)fakedevice[i]);
		ui.IDS_DEVSTR->addItem(tText);
	}
	...
	ui.IDS_LVFLIPSTR->setChecked(config.GetBoolean(VideoOutVFlipConfigKey, FALSE));
	ui.IDS_RVFLIPSTR->setChecked(config.GetBoolean(VideoInVFlipConfigKey, FALSE));
	...
	ui.IDS_DEVCHSTR->setValue(config.GetInteger(VideoSourceConfigKey, -1));
...
void CSettingsDlg::saveIni()
{
	...
	t_str = ui.IDS_UNAMESTR->text();
	p_endpoint->SetLocalUserName(t_str.toUtf8().data());
	config.SetString(UsernameConfigKey, t_str.toUtf8().data()); 
	
	bool b_val = ui.IDS_D242STR->isChecked();
	config.SetBoolean(NoTunnelingConfigKey, b_val);
	p_endpoint->m_fDoH245Tunnelling = b_val;
	p_endpoint->DisableH245Tunnelling(b_val);
	...
	config.SetBoolean(AutoTransmitVideoConfigKey, ui.IDS_VSENDSTR->isChecked());
	config.SetBoolean(AutoReceiveVideoConfigKey, ui.IDS_VRCVDSTR->isChecked());
	config.SetBoolean(VideoLocalConfigKey, ui.IDS_VLOCSTR->isChecked());
	...
	config.SetInteger(VideoSourceConfigKey, ui.IDS_DEVCHSTR->value());
	...
	config.SetBoolean(UseGatekeeperConfigKey, ui.IDS_GKUSESTR->isChecked());
	config.SetBoolean(RequireGatekeeperConfigKey, ui.IDS_GKRECSTR->isChecked());
	QString gkhost = ui.IDS_GKHOSTLOCSTR_STR->text();
	config.SetString(GatekeeperHostConfigKey, gkhost.toAscii().data()); 
	config.SetString(GatekeeperPassConfigKey, ui.IDS_GKPASSSTR->text().toAscii().data()); 

	int imode=-1;
	if(ui.IDS_GKAUTOLOCSTR->isChecked())
		imode = 0;
	if(ui.IDS_GKHOSTLOCSTR->isChecked())
		imode = 1;
	if(ui.IDS_GKIDLOCSTR->isChecked())
		imode = 2;
	config.SetInteger(DiscoverGatekeeperConfigKey, imode);
